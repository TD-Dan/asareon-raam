 ...
 * ---
 * ## Mandate
 * This class is a proper test double that inherits from the `ImportExportManager` class.
 * It overrides all functions to prevent real file I/O during tests and uses "spy" properties
 * to allow tests to verify that the correct logic paths were triggered.
 *
 * ---
 * ## Dependencies
 * - `app.auf.ImportExportManager` (the class it inherits from)
 *
 * @version 2.2
 * @since 2025-08-15
 */
class FakeImportExportManager(
    // --- FIX IS HERE: It now accepts the platform dependency, just like the real class. ---
    platform: PlatformDependencies
) : ImportExportManager(
    platform = platform,
    jsonParser = JsonProvider.appJson
) {
    // Spy properties to track method calls
    ...
    // Control property to dictate the output of a method for testing different scenarios
